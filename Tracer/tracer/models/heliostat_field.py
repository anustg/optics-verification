...
class TowerScene(Assembly):
	def __init__(self,one_mirror=False):
		self.one_mirror=one_mirror

	def __call__(self, field, layout, aiming, tracking,receiver, mount_rec):
		...
		if self.one_mirror:
			...
			if field == None:
				...
				field(layout)
				heliostats=Assembly(objects=field._heliostats)		
				tracking.aim_to_sun(-layout.pos, aiming.aiming_points)
				tracking(field)
				if receiver==None:
...
class KnownField(FieldLayout):
	def __init__(self, filename,pos=N.zeros(3),foc=N.zeros(3)):
		...
		if (any(pos) == 0. and any(foc)==0.):
			self.layout=N.loadtxt(filename, delimiter=',')

			self.pos=self.layout[:,:3]

			self.focals=self.layout[:,3]
...
class HeliostatGenerator:
	...
	def __init__(self, width, height, absorptivity, sigma_xy,slope, curved=True):
		"""
...
class SingleMirrorGenerator:
	def __init__(self, width, height, absorptivity, sigma_xy,slope,index=-1,pos=N.zeros(3),foc=N.zeros(3),curved=True):
		"""
		Generates one mirror, (either at a defined point or from a field) 
		it's a rectangular one-sided mirror (either curved or flat), 
		initially pointing downward - for safety reasons, of course :)
        
		Arguments:
		
		width, height - The width and height, respectively, of the mirror
		absorptivity - part of incident energy absorbed by the heliostat
		sigma_xy: slope error (mrad)
		slope: the shape of slope error, options: (1)pillbox (2) normal (3) ns (normal simplified)
		curved - boolean, True or False  
	...
	def __call__(self, layout):
		...
		if self.curved:
			print ''
			print "I'm generating one curved mirror"
...
class MultiFixedPointsAiming(AimingStrategy):
	...
	def __init__(self, filename):
		...
		Arguments:
		filename - filename of the file (.csv) that listed the coordinates and 
			focal lengths of the known field and aiming points
				layout[:,4]- x coordinates of the aiming points
				layout[:,5]- y coordinates of the aiming points
				layout[:,6]- z coordinates of the aiming points			
		...
		self.aiming_points=self.layout[:,4:]
...
class AzElTrackings(TrackingSystem):
	...
	def __call__(self,heliostats):
		...
		if self.one_mirror:
			...
			hstat_az=N.arctan2(norm_x, norm_y)							
