...
class TracerEngineMP(TracerEngine):
	...
	def trace(self, source):
		...
		self.ray_tracer(source, itmax, self.minener, tree=True)
	...
	def multi_ray_sim(self, sources, procs=8):
		...
		# Creates a pool of processes and makes them raytrace one different source each. The resm list returned is aa list of copies of the original engine post raytrace.
